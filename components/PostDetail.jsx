...
const PostDetail = ({ post }) => {
  const getContentFragment = (index, text, obj, type) => {
    ...
    switch (type) {
      ...
      case 'paragraph':
        return <p key={index} className="mb-8 scroll-smooth">{modifiedText.map((item, i) => <React.Fragment key={i}>{item}</React.Fragment>)}</p>;
      case 'heading-four':
  ...
  return (
    <div   className={`bg-white shadow-lg rounded-lg lg:p-8 pb-12 mb-8 h-96 overflow-y-auto dark:text-black dark:bg-slate-800 ${styles.scroll} `}>
      ...
      <div className='px-4 lg:px-0 dark:text-white'>
        <div className='flex items-center mb-8 w-full'>
          <div className="flex items-center mb-4 lg:mb-0 w-full lg:w-auto mr-8 items-center">
            <Image
              ...
              height={30}
            />
            <p className="inline align-middle text-gray-700 ml-2 font-medium text-lg dark:text-white">{post.author.name}</p>
          </div>
